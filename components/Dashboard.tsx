...
import { Play, Pause, CheckCircle2, AlertCircle, Clock, ChevronRight, UserCheck, Zap, Box, RefreshCw, Shield, Terminal, Download, FileJson, FileCode } from 'lucide-react';
...
const Dashboard: React.FC<DashboardProps> = ({ flow, onTaskUpdate, theme }) => {
  ...
  const downloadArtifact = (task: Task) => {
    ...
    a.href = url;
    a.download = `Artefact_${task.id}.${ext}`;
    a.click();
  };
          ...
          <div className="flex-1 overflow-y-auto pr-4 space-y-3 custom-scrollbar">
            {flow.tasks.map(task => (
              <div 
                ...
                className={`p-5 rounded-[2rem] border cursor-pointer transition-all duration-300 group ${selectedTaskId === task.id ? 'border-blue-500 bg-blue-600/5' : (theme === 'dark' ? 'border-white/5 bg-white/[0.02]' : 'border-slate-200 bg-white shadow-sm')}`}
              >
                <div className="flex items-center justify-between">
                  ...
                  {task.status === 'completed' && (
                    <button 
                      ...
                      className="p-2 hover:bg-white/10 rounded-lg text-slate-500 hover:text-blue-500"
                    >
                      <Download size={14} />
                    </button>
                  )}
                ...
                <div className="p-8 pb-4 shrink-0 border-b border-white/5">
                  ...
                  {selectedTask.status === 'completed' && (
                    <button 
                      ...
                      className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-blue-500 hover:underline mb-2"
                    >
                      <Download size={12} /> Download Artefact
                    </button>
                  )}
                </div>
